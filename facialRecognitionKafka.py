...
broker = os.getenv('BROKER', '192.168.1.11:9092')
...
def publish_message(p_instance, topic_name, key, value):
    try:
        key_bytes = bytes(key, encoding='utf-8')
        value_bytes = bytes(value, encoding='utf-8')
        p_instance.send(topic_name, key=key_bytes, value=value_bytes)
        p_instance.flush()
...
def connect_kafka_producer(broker_url):
    _producer = None
    try:
        _producer = KafkaProducer(bootstrap_servers=[broker_url], api_version=(0, 10))
    except Exception as ex:
